...
public class Report4 {
    ...
    int[] arr = {1,2,3,};
    int[] arr = new int[5];
    int[] arr = new int[5]{1,2,3,4,5}; //두번째 int 뒤 대괄호에 5를 적으면 안된다
    int arr[5]; //배열의 크기 선언은 int[]arr = new int[5];로 한다
    int[] arr[] = new int[3][];
...
//코드를 완성하세요. 다만 Math.random()을 사용했기 때문에 실행 결과 예시와 다를 수 있습니다.
    class Exercise5_5{
        public static void main(String[] args) {
            ...
            for (int i = 0; i < ballArr.length; i++) {
                System.out.printf("%-4d", ballArr[i]);
            }
            System.out.println();
